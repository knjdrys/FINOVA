...
export interface EssentialExpenseBreakdown {
  ...
  /** Average monthly essential-category spend over observed history (up to 6 months). */
...
export class EmergencyFundEngine {
  ...
  static deriveEssentialMonthlyExpenses(args: {
    ...
    for (const c of args.commitments) {
      // Generated occurrences are skipped: their rule already contributes via
      // the recurring loop below — counting both double/triple-counts the
      // same bill. Settled bills (COMPLETED/AUTO_POSTED) are history, not
      // future monthly need — only outstanding obligations project forward.
      if (c.relatedRecurringTransactionId || c.isAutoGenerated) continue;
      if (c.status === 'COMPLETED' || c.status === 'CANCELLED' || c.status === 'AUTO_POSTED') continue;
      if (c.priority === 'ESSENTIAL' && c.direction !== 'INFLOW') {
    ...
    for (const r of args.recurring) {
      // Legacy rules predate the priority field: for a SAFETY instrument the
      // conservative default is ESSENTIAL (excluding them understates the
      // cushion — the dangerous direction).
      const essential = r.priority === 'ESSENTIAL' || r.priority === undefined;
      if (r.isActive && r.type !== 'INCOME' && essential) {
    ...
    if (essentialTx.length > 0) {
      ...
      // Divide by the OBSERVED month span (first activity → today), not a
      // fixed 6: a new user with one month of history would otherwise see
      // their burn understated 6x, while zero-spend months inside the span
      // still count as real zeros.
      const firstDate = essentialTx.reduce((m, t) => (t.date < m ? t.date : m), essentialTx[0].date);
      const spanMonths = Math.max(
        1,
        (Number(todayISO.slice(0, 4)) - Number(firstDate.slice(0, 4))) * 12 +
          (Number(todayISO.slice(5, 7)) - Number(firstDate.slice(5, 7)) + 1)
      );
      actualAvgMonthly = Math.round(total / spanMonths);
    }
