  ...
  public static async loadStateFromCloud(user: AuthUserProfile): Promise<FinovaState | null> {
    ...
    try {
      ...
      const transactions: Transaction[] = (txData || []).map((t) => ({
        ...
        sourceCommitmentId: t.source_commitment_id || undefined,
      ...
      const commitments: MoneyCommitment[] = (commitmentsData || []).map((c) => ({
        ...
        goalId: c.goal_id || undefined,
        relatedRecurringTransactionId: c.related_recurring_transaction_id || undefined,
        lastAutoPostedAt: c.last_auto_posted_at || undefined,
      ...
      const recurring: RecurringTransaction[] = (recurringData || []).map((r) => ({
        ...
        // Nullable on purpose: legacy rules predate the flag ("unknown",
        // not "essential") — the emergency-fund engine falls back instead.
  ...
  public static async syncStateToCloud(state: FinovaState, user: AuthUserProfile): Promise<boolean> {
    ...
    try {
          ...
          ? (() => {
              ...
              return state.transactions.map((tx) => {
                ...
                return {
                  ...
                  source_commitment_id: tx.sourceCommitmentId ? toValidUuid(tx.sourceCommitmentId) : null,
                  updated_at: new Date().toISOString(),
                };
              });
            })()
      ...
      const commitmentPayloads = state.commitments.map((c) => ({
        ...
        goal_id: c.goalId ? toValidUuid(c.goalId) : null,
        related_recurring_transaction_id: c.relatedRecurringTransactionId
          ? toValidUuid(c.relatedRecurringTransactionId)
          : null,
        last_auto_posted_at: c.lastAutoPostedAt || null,
        notes: c.notes || null,
        updated_at: new Date().toISOString(),
      }));
      const recurringPayloads = state.recurring.map((r) => ({
        ...
        destination_account_id: r.destinationAccountId ? toValidUuid(r.destinationAccountId) : null,
        updated_at: new Date().toISOString(),
      }));
